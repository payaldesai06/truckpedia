...
export default {
  name: 'AccessorialDeductionPrompt',
  ...
  computed: {
    ...
    promptTitle() {
      return this.isEditMode
        ? `Edit Accessorial Deduction`
        : `New Accessorial Deduction`;
    },
  },
  ...
  methods: {
    onClose(isNewAdded = false) {
      ...
      if (!isNewAdded) {
        this.$emit('accessorial-deduction::added', null);
      }
      this.$emit('close');
    },
    async onAccept() {
      try {
        ...
        if (this.isEditMode) {
          ...
          const {
            ...
            'accounting/updateAccessorialDeduction',
            payload
          );
          if (result) {
            this.$emit('accessorial-deduction::updated', {
              id: this.id,
              name: this.name,
            ...
            this.$vs.notify({
              color: 'primary',
              title: 'Accessorial Deduction',
              text: 'Accessorial deduction updated successfully.',
            });
          }
        } else {
          let payload = {
          ...
          const { data } = await this.$store.dispatch(
            'accounting/createAccessorialDeduction',
            payload
          );
          if (data && data.payload) {
            const accessorialDeductionFee = {
              ...payload,
              id: data.payload.id,
              commissionable: this.commissionable,
            };
            this.$emit(
              'accessorial-deduction::added',
            ...
            this.$vs.notify({
              color: 'primary',
              title: 'Accessorial Deduction',
              text: 'Accessorial deduction added successfully.',
            });
          }
        }
      } catch (error) {
