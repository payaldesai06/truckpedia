<template>
  <div class="gantt__layout">
    <div class="flex">
      <div
        class="unassigned-loads-col"
        ref="groupedUnassignedLoads"
        v-if="headerData[0] === 'Driver Name'"
      >
        <div class="flex items-center">
          <h5>Unassigned Loads</h5>
          <vx-tooltip
            text="Drag each card to assign a driver"
            position="bottom"
          >
            <vs-icon icon="info_outline" class="ml-3 cursor-default" />
          </vx-tooltip>
        </div>
        <template v-if="groupedUnassignedLoads.length">
          <div class="mt-3 overflow-y-auto max-h-81vh">
            <div
              v-for="(loadParent, index) in groupedUnassignedLoads"
              :key="index"
            >
              <span class="created-date mb-3">{{ loadParent.groupDate }}</span>
              <LoadCard
                v-for="load in loadParent.loads"
                :key="load.key"
                :load="load"
                :isDraggable="true"
                :isTimeLineCard="false"
                @click.native="$emit('onLoadCardClick', load)"
                @onLoadCardDragEnd="assignLoadToDriver"
              />
            </div>
          </div>
        </template>
      ...
      <div class="chart-panel w-full">
        ...
        <chart-header
          ...
          @onDriverChange="(drivers) => (selectedDrivers = drivers)"
        />
        ...
        <div
          ...
          class="gantt__area"
          ref="chartContainer"
          @scroll.passive="containerScroll"
          :style="areaStyleObj"
        >
...
export default {
  name: 'GanttChart',
  components: { ChartHeader, ChartSide, ChartContainer, LoadCard },
  provide() {
    return {
  ...
  computed: {
    areaStyleObj() {
      ...
      let styleObj = {
        'max-height': `${this.chartMaxHeight}px`,
        'min-height': `${this.chartMaxHeight}px`,
        'overflow-y': 'auto',
      };
    ...
    chartData() {
      ...
      const filteredGanttData =
        this.selectedDrivers.length > 0
          ? this.ganttData.filter((data) =>
              this.selectedDrivers.includes(data.driverId)
            )
          : ganttData;
      ...
      return [...filteredGanttData, ...dummyData.slice(0, 10 - dataLength)];
    },
    chartPropertySync() {
      return {
  ...
  mounted() {
    ...
    if (this.$refs.chartContainer) {
      this.$refs.chartContainer.scrollTo(0, this.currentScrollPosition);
    }
  },
  methods: {
    containerScroll($event) {
      this.$emit('onScrollPositionChange', $event.target.scrollTop);
    },
    assignLoadToDriver(load) {
      this.$emit('onLoadAssign', load);
    },
    assignDriverForLoad(driver) {
