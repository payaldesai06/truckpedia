...
export default {
  ...
  methods: {
    ...
    async fetchFolders() {
      const { data } = await this.$store.dispatch("trailersV2/fetchFolders");
      this.foldersList = (data || {}).payload || [];
      return data;
    },
    async fetchFiles() {
      const { data } = await this.$store.dispatch(
        "trailersV2/fetchFiles",
        this.trailerId
      );
    ...
    async addFolder({ isEdit, id, folderName }) {
      try {
        isEdit
          ? await this.$store.dispatch("trailersV2/updateFolder", {
              folderId: id,
              folderName,
            })
          : await this.$store.dispatch("trailersV2/createFolder", {
              folderName,
            });
    ...
    async deleteFile({ file }) {
      ...
      try {
        const { id } = file || {};
        await this.$store.dispatch("trailersV2/deleteFile", id);
    ...
    async generateS3NameAndURL(files) {
      try {
        ...
        const {
          data: { result, payload },
        } = await this.$store.dispatch("trailersV2/uploadAttachment", {
          files: allFiles,
        });
    ...
    async uploadFiles({ folderId, files }) {
      try {
        ...
        await this.$store.dispatch("trailersV2/uploadFiles", payload);
        await this.fetchFiles();
