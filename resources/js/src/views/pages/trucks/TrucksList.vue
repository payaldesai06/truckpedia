    ...
    <div class="vx-row m-0 flex justify-between items-center w-full">
      ...
      <div class="vx-col pr-0 flex">
        ...
        <vs-button
          ...
          color="primary"
          icon="local_shipping"
          @click.prevent="redirectToAddTruck"
        >
          ADD TRUCK
        </vs-button>
      </div>
      ...
      <div class="vx-col w-full">
        ...
        >
          ...
          <Column
            ...
            :sortable="false"
          >
            <template #body="{ data: { truckTypes } }">
              <div>
                <span v-text="getTruckTypes(truckTypes)" />
              </div>
            </template>
            <template #filter="{}">
              <Dropdown
                v-model="filter['truckType'].value"
                :options="truckTypes"
...
export default {
  ...
  methods: {
    async fetchTrucks(pageSize = this.defaultPageSize, currentPage = 1) {
      try {
        this.loading = true;
        const payload = {
          pageSize,
    ...
    initLocalFilters() {
      ...
      const validRoutes = ['CreateTrucks', 'UpdateTrucks'];
      const truckFilters =
        (this.$store.state.localFilters &&
          this.$store.state.localFilters.truckFilters) ||
        {};
      ...
      if (
        ...
        Object.keys(truckFilters).forEach((key) => {
          if (truckFilters[key]) {
            if (typeof truckFilters[key] === 'object') {
              this.filter[key].value = truckFilters[key].value;
            } else {
              this.filter[key] = truckFilters[key];
            }
          }
        });
      } else {
    ...
    redirectToAddTruck() {
      ...
      this.$router.push({ name: 'CreateTrucks' }).catch(() => true);
    },
    editTruck(data) {
      if (!(data || {}).id) return;
      this.saveLocalFilters();
    ...
    getTruckTypes(selectedTruckTypes) {
      selectedTruckTypes = selectedTruckTypes || [];
      const selectedLabels = this.truckTypes
        .filter((truckType) => selectedTruckTypes.includes(truckType.value))
        .map((truckType) => truckType.title);
    ...
    async exportExcel() {
      try {
        ...
        const data = await this.$store.dispatch('trucksV2/fetchAllTrucks', {
          payload,
        });
