...
class CreateScheduleRequest extends FormRequest {
  ...
  public function rules(): array {
    ...
    $rules = [
      'truckIds' => 'required|array|min:1',
      'truckIds.*.truckId' => 'required_with:truckIds|integer|exists:trucks,id,company_id,' . $companyId,
      ...
      'equipmentTypes' => 'required|array|min:1',
      'equipmentTypes.*' => 'string|in:' .
        implode(',', config('constant.planning.schedule.equipmentTypes')),
      'manualLanes' => 'nullable|array',
      ...
      'manualLanes.*.origin.statesArray.*' => 'string|size:2|regex:/^[A-Z]{2}$/|in:' . implode(',', config('constant.usStatesIntMapping')),
      ...
      'manualLanes.*.destination.statesArray.*' => 'string|size:2|regex:/^[A-Z]{2}$/|in:' . implode(',', config('constant.usStatesIntMapping')),
  ...
  public function messages(): array {
    return [
      'truckIds.*.truckId' => 'The truckIds truckId field is required',
      ...
      'equipmentTypes.*.in' => 'The equipmentTypes is supported only ' .
        implode(',', config('constant.planning.schedule.equipmentTypes')),
      'equipmentTypes.*.distinct' => 'The equipmentTypes value field has a duplicate value.',
      'equipmentTypes.*.string' => 'The equipmentTypes value field must be a string.',
      ...
      'manualLanes.*.destination.latitude.numeric' => 'The destination latitude field must be numeric.',
      ...
      'manualLanes.*.destination.longitude.numeric' => 'The destination longitude field must be numeric.',
      ...
      'manualLanes.*.targetPrice.numeric' => 'The targetPrice field must be numeric.',
      'manualLanes.*.targetPrice.between' => 'The targetPrice field must be between 0 and 999999.99',
      'manualLanes.*.targetRpm.numeric' => 'The targetRpm field must be numeric.',
      'manualLanes.*.targetRpm.between' => 'The targetRpm field must be between 0 and 99.9999',
      'manualLanes.*.targetWeight.numeric' => 'The targetWeight field must be an integer.',
