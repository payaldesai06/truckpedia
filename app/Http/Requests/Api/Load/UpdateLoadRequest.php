...
class UpdateLoadRequest extends FormRequest {
  ...
  public function rules(): array {
    $companyType = AdminCompanyDetail::select('company_type')->where('id', Auth::user()->admin_company_detail_id)->first()->company_type ?? null;
    ...
    $existingDriverUsers = LoadDriver::select('id', 'load_id')
      ->where('load_id', request('load'))
      ->with('driverAsUsers:id')
      ->get()->pluck('driverAsUsers')->flatten()->pluck('id')->toArray();

    return [
      'customer_id' => $customerRule,
      'status' => 'nullable|string|in:' . implode(',', config('constant.status.load')),
      ...
      'calculate_using' => [
        'required',
        "in:" . config('constant.calculate_using.manual') . "," . config('constant.calculate_using.google_maps') . "," . config('constant.calculate_using.p'),
      ],
      ...
      'shippers.*.arrivedLocalDatetime' => ['nullable', 'date_format:' . config('constant.date_time_format')],
      ...
      'load_legs.*.location_name' => 'required',
      ...
      'drivers.*.driver_ids' => [
        function ($attribute, $value, $fail) use ($existingDriverUsers) {
          $loadStatus = Load::where('id', request('load'))->select('status')->first();
          ...
          //check unapprove drivers with new drivers only not with existing drivers
          $unapprovedDrivers = $driverDetails->where(
            'approval_status',
            array_search('notApproved', config('constant.driver.approvalStatus'))
          )->whereNotIn('user_id', $existingDriverUsers)->pluck('name')->toArray();
          ...
          if (
            $loadStatus->status == config('constant.status.load.canceled')
            && $driverDetails->whereNotIn('user_id', $existingDriverUsers)->isNotEmpty()
          ) {
            $fail(config('response.load.dispatchDriverToCanceledLoad'));
          }
        },
        '',
        'nullable',
      ...
      'thirdPartyCarrier.charges.hauling_fee' => 'nullable|numeric',
      'thirdPartyCarrier.charges.hauling_fee_rate' => 'nullable|numeric',
      ...
      'thirdPartyCarrier.charges.haulingFeeQty' => 'nullable|required_if:thirdPartyCarrier.charges.hauling_fee_type,' .
        config('constant.hauling_fees_type.truck_hour') .
        '|numeric|between:0,9999.99',
      'thirdPartyCarrier.charges.fuel_surcharge' => 'nullable|numeric',
      'thirdPartyCarrier.charges.fuel_surcharge_rate' => 'nullable|numeric',
      ...
      'thirdPartyCarrier.charges.accessorial_fee.*.id' => 'numeric|exists:accessorial_fees,id,admin_company_detail_id,' . getAuthAdminCompanyId(),
      'thirdPartyCarrier.charges.accessorial_fee.*.rate' => 'nullable|numeric',
      'thirdPartyCarrier.charges.accessorial_deduction' => 'nullable',
      'thirdPartyCarrier.charges.accessorial_deduction.*.id' => 'numeric|exists:accessorial_deductions,id,admin_company_detail_id,' . getAuthAdminCompanyId(),
      'thirdPartyCarrier.charges.accessorial_deduction.*.rate' => 'nullable|numeric',
      ...
      'multiPurposeStops.*.multiPurposeStopId' => 'nullable|integer',
      ...
      'notes.*.id' => 'nullable|integer|exists:load_notes,id,load_id,' . request('load'),
  ...
  public function messages(): array {
    ...
    return [
      'status.in' => 'The status must be in ' . implode(',', config('constant.status.load')),
      ...
      'shippers.*.dropTrailerType.string' => 'The Shipper dropTrailerType field must be a string.',
      'shippers.*.hookTrailerType.string' => 'The Shipper hookTrailerType field must be a string.',
      'shippers.*.dropTrailerType.in' => 'The Shipper dropTrailerType field must be in ' . $dropHookTrailerType,
      'shippers.*.hookTrailerType.in' => 'The Shipper hookTrailerType field must be in ' . $dropHookTrailerType,
      'shippers.*.dropTrailerId.exists' => 'The Shipper dropTrailerId field must be valid.',
      'shippers.*.dropTrailerId.integer' => 'The Shipper dropTrailerId field must be an integer.',
      'shippers.*.hookTrailerId.exists' => 'The Shipper hookTrailerId field must be valid.',
      'shippers.*.hookTrailerId.integer' => 'The Shipper hookTrailerId field must be an integer.',

      'shippers.*.freightEquipmentType.string' => 'The shippers freightEquipmentType must be a string type.',
      'shippers.*.freightEquipmentType.max' => 'The shippers freightEquipmentType maximum length is 255.',
      ...
      'multiPurposeStops.*.multiPurposeStopId.integer' => 'The multiPurposeStops multiPurposeStopId field must be an integer.',
      ...
      'notes.*.id.exists' => 'The notes id field must be valid.',
      'notes.*.id.integer' => 'The notes id field is supported only integer type.',
